...
class BeamSearchScorerCPU(BeamScorer):
    r"""
    ...
    def process(
        ...
        if beam_indices is None:
            ...
            beam_hyps = custom_beam_search_cpu.BeamSearchProcess(
                next_tokens,
                next_scores,
                next_indices,
                beam_hyps,
            ...
            next_beam_scores = next_beam_scores.to(device)
            next_beam_tokens = next_beam_tokens.to(device)
            next_beam_indices = next_beam_indices.to(device)
            # print(f"[beam_hyps]{beam_hyps}")
            for idx, beam_hyp in enumerate(self._beam_hyps):
                new_beams = []
                ...
                for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                    ...
                    if (eos_token_id is not None) and (next_token.item() in eos_token_id):
                        ...
                        if beam_indices is not None:
                            ...
                            beam_index = None
                        # print(f"[baseline][{cur_len}][{batch_idx}]", end=" ")
                        beam_hyp.add(
...
class BeamSearchScorerGPU(BeamScorer):
    r"""
    [`BeamScorer`] implementing standard beam search decoding.
    Adapted in part from [Facebook's XLM beam search
    code](https://github.com/facebookresearch/XLM/blob/9e6f6814d17be4fe5b15f2e6c43eb2b2d76daeb4/src/model/transformer.py#L529).
    ...
    def process(
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            if self._done[batch_idx]:
                ...
                # We need to push out this batch_idx and replace this with a new "group" of beams 
                next_beam_scores[batch_idx, :] = 0
                next_beam_tokens[batch_idx, :] = pad_token_id
                next_beam_indices[batch_idx, :] = 0
                continue
