...
class GenerationMixin:
    ...
    @measure_times
    def beam_search_partitioned(
        self,
        input_ids_gpu: torch.LongTensor,
        input_ids_cpu: torch.LongTensor,
        beam_scorer_gpu: BeamScorer,
        beam_scorer_cpu: BeamScorer,
        logits_processor_gpu: Optional[LogitsProcessorList] = None,
        logits_processor_cpu: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        max_length: Optional[int] = None,
        pad_token_id: Optional[int] = None,
        eos_token_id: Optional[Union[int, List[int]]] = None,
        ...
        batch_size_gpu = len(beam_scorer_gpu._beam_hyps)
        batch_size_cpu = len(beam_scorer_cpu._beam_hyps)
        batch_size_total = batch_size_cpu + batch_size_gpu
        # num_beams is same across gpu and cpu sides...
        num_beams = beam_scorer_gpu.num_beams

        batch_beam_size_gpu, cur_len_gpu = input_ids_gpu.shape
        batch_beam_size_cpu, cur_len_cpu = input_ids_cpu.shape
        batch_beam_size_total = batch_beam_size_gpu + batch_beam_size_cpu

        if num_beams * batch_size_gpu != batch_beam_size_gpu:
            raise ValueError(
                f"Batch dimension of `input_ids_gpu` should be {num_beams * batch_size_gpu}, but is {batch_beam_size_gpu}."
            )

        if num_beams * batch_size_cpu != batch_beam_size_cpu:
            raise ValueError(
                f"Batch dimension of `input_ids_cpu` should be {num_beams * batch_size_cpu}, but is {batch_beam_size_cpu}."
            )

        # init attention / hidden states / scores tuples
        scores = () if (return_dict_in_generate and output_scores) else None
        beam_indices = (
            tuple(() for _ in range(batch_beam_size_total)) if (return_dict_in_generate and output_scores) else None
        )
        decoder_attentions = () if (return_dict_in_generate and output_attentions) else None
        cross_attentions = () if (return_dict_in_generate and output_attentions) else None
        decoder_hidden_states = () if (return_dict_in_generate and output_hidden_states) else None
        ...
        while True:
            ...
            if done_gpu and done_cpu:
                ...
                # TBD what to do here; maybe parition more of the gpu data bundle onto the CPU
                # or continue running entirely on the GPU
                is_cpu = False
                is_gpu = True
                sequence_outputs_cpu = beam_scorer_cpu.finalize(
                    input_ids_cpu,
