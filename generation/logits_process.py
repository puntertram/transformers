...
class NoRepeatNGramLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if cur_len > self.ngram_size:
            ...
            banned_tokens = cpu_accel.NoRepeatNGramLogitsProcessor(int(self.ngram_size), input_ids, scores, int(scores.shape[0]), 6)
            
            # num_batch_hypotheses = scores.shape[0]
            # cur_len = input_ids.shape[-1]
            # banned_batch_tokens = _calc_banned_ngram_tokens(self.ngram_size, input_ids, num_batch_hypotheses, cur_len)

            # for i, banned_tokens_ in enumerate(banned_batch_tokens):
            #     scores[i, banned_tokens_] = -10000.0

            # assert banned_tokens == banned_batch_tokens
