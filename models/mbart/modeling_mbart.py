...
from workload_partitioner import WorkloadBundle, WorkloadPartitioner, PARTITION_RESIDENT_DEVICES, PARTITION_TYPES
...
class MBartWorkloadPartitioner(WorkloadPartitioner):
    def __init__(self, partition_type: PARTITION_TYPES) -> None:
        super().__init__(partition_type)
        if partition_type == PARTITION_TYPES.CPU_GPU:
            self.gpu_bundle = MBartWorkloadBundle(PARTITION_RESIDENT_DEVICES.GPU)
            self.cpu_bundle = MBartWorkloadBundle(PARTITION_RESIDENT_DEVICES.CPU)
        elif partition_type == PARTITION_TYPES.GPU:
            self.gpu_bundle = MBartWorkloadBundle(PARTITION_RESIDENT_DEVICES.GPU)
        elif partition_type == PARTITION_TYPES.BASELINE:
            self.bundle = MBartWorkloadBundle(PARTITION_RESIDENT_DEVICES.BASELINE)
    

# To be moved to modeling_mbart.py
class MBartWorkloadBundle(WorkloadBundle):
    def __init__(self, resident_device: PARTITION_RESIDENT_DEVICES) -> None:
        super().__init__(resident_device)
        self.model_kwargs = {}
    
    def set_model_kwargs(self, key, value):
        self.model_kwargs[key] = value



class MBartForConditionalGenerationPartitioned(MBartPreTrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [
        r"final_logits_bias",
        r"encoder.version",
    ...
    def __init__(self, config: MBartConfig):
        ...
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
        self.workload_partitioner = MBartWorkloadPartitioner()
