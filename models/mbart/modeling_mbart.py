...
class MBartAttention(nn.Module):
    ...
    def forward(
        ...
        # get key, value proj
            ...
            # parallelize torch bmm
            # number_of_streams = 1
            # bmm_outs = [None]*number_of_streams
            # streams = [None]*number_of_streams
            # for i in range(number_of_streams):
            #     streams[i] = torch.cuda.Stream()
            # for i in range(number_of_streams):
            #     with torch.cuda.stream(streams[i]):
            #         bmm_outs[i] = torch.bmm(query_states[(i * (proj_shape[0] // number_of_streams)): ((i + 1) * (proj_shape[0] // number_of_streams))], key_states.transpose(1, 2)[(i * (proj_shape[0] // number_of_streams)): ((i + 1) * (proj_shape[0] // number_of_streams))])
            # for i in range(number_of_streams):
            #     streams[i].synchronize()
            #     del streams[i]
            # torch.cuda.synchronize()
            # with torch.autocast(device_type="cuda", dtype=torch.float16):
            attn_weights_orig = torch.bmm(query_states, key_states.transpose(1, 2))
            attn_weights_orig = attn_weights_orig.type(torch.float32)
            # attn_weights_new = torch.stack(bmm_outs)
            attn_weights = attn_weights_orig
            # assert (attn_weights_new == attn_weights_orig).all().item()
        ...
        # attn_output = attn_output.type(torch.float32)
...
class MBartEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states1 = self.fc2(hidden_states)
        hidden_states = nn.functional.dropout(hidden_states1, p=self.dropout, training=self.training)
        hidden_states = residual + hidden_states
